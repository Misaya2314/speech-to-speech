...
根据提供的设计草图，界面将包括以下主要部分：

1. **顶部导航栏**：包含应用标题、设置按钮和其他控制选项
2. **主对话区域**：显示对话历史，包括用户输入和 AI 回复
3. **控制面板**：包含开始/停止按钮、麦克风状态指示器、语言选择下拉菜单
4. **侧边栏**：可展开/收起，包含高级设置和模型配置选项
5. **状态栏**：显示系统状态、模型加载进度等信息
