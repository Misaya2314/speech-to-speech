...
class FacebookMMSTTSHandler(BaseHandler):
    ...
    def load_model(self, language_code):
        ...
        try:
            ...
            log.warning(f"Unsupported language: {language_code}. Falling back to English.")
            self.load_model("en")
    ...
    def generate_audio(self, text):
        ...
        if not text:
            log.warning("Received empty text input")
            return None

        try:
            log.debug(f"Tokenizing text: {text}")
            log.debug(f"Current language: {self.language}")
            log.debug(f"Tokenizer: {self.tokenizer}")
            
            inputs = self.tokenizer(text, return_tensors="pt", padding=True, truncation=True)
            input_ids = inputs.input_ids.to(self.device).long()
            attention_mask = inputs.attention_mask.to(self.device)
            
            log.debug(f"Input IDs shape: {input_ids.shape}, dtype: {input_ids.dtype}")
            log.debug(f"Input IDs: {input_ids}")
            
            if input_ids.numel() == 0:
                log.error("Input IDs tensor is empty")
                return None

            with torch.no_grad():
                output = self.model(input_ids=input_ids, attention_mask=attention_mask)
            
            log.debug(f"Output waveform shape: {output.waveform.shape}")
            return output.waveform
        except Exception as e:
            log.error(f"Error in generate_audio: {str(e)}")
    ...
    def process(self, llm_sentence):
        ...
        log.debug(f"Processing text: {llm_sentence}")
        log.debug(f"Language code: {language_code}")

        if language_code is not None and self.language != language_code:
            try:
                log.info(f"Switching language from {self.language} to {language_code}")
                ...
                log.warning(f"Unsupported language: {language_code}")
        ...
        log.debug(f"Raw audio shape: {audio_numpy.shape}, dtype: {audio_numpy.dtype}")
        
        audio_resampled = librosa.resample(audio_numpy, orig_sr=self.model.config.sampling_rate, target_sr=16000)
        log.debug(f"Resampled audio shape: {audio_resampled.shape}, dtype: {audio_resampled.dtype}")
        
        audio_int16 = (audio_resampled * 32768).astype(np.int16)
        log.debug(f"Final audio shape: {audio_int16.shape}, dtype: {audio_int16.dtype}")
